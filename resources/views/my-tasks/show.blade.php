...
@section('content')
    <div class="container">
        <a href="{{ route('client.my-tasks.index') }}" class="btn btn-info mb-3">Back</a>
        <a href="{{ route('client.my-tasks.closeTask', $task->id) }}" class="btn btn-warning mb-3">Close task</a>
        <table class="table">
            <tbody>
                <tr>
                  <th>Title</th>
        ...
        @endif
        <form action="{{ route('client.my-tasks.sendMessage', $task->id) }}" method="POST" class="mt-3">
          @csrf
